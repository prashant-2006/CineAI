...
import { Loader2, Clapperboard, Star, X, Eye } from "lucide-react";
...
export default function WatchlistPage() {
  ...
  // 👇 The new Tactical Pipeline: Add to Activity, Remove from Target List
  const markAsWatched = async (imdbID) => {
    try {
      ...
      await removeMovie(imdbID); // Clear it from this screen
    } catch (err) {
              ...
              <div key={movie.imdbID} className="group relative bg-zinc-950 border border-zinc-900 overflow-hidden flex flex-col h-full hover:border-zinc-700 transition-colors">
                <div className="aspect-[2/3] w-full bg-zinc-900 relative">
                  ...
                  <div className="absolute inset-0 bg-black/90 opacity-0 group-hover:opacity-100 transition-all duration-300 flex flex-col items-center justify-center gap-2 p-3 z-20">
                    ...
                    <button onClick={() => markAsWatched(movie.imdbID)} className="w-full flex justify-center items-center gap-2 border border-zinc-400 text-zinc-200 px-3 py-2.5 font-oswald uppercase tracking-widest text-[11px] hover:bg-white hover:text-black transition-all">
                      <Eye size={16} /> Mark Watched
                    </button>

                    <button onClick={() => removeMovie(movie.imdbID)} className="w-full flex justify-center items-center gap-2 border border-red-900/50 text-red-500 px-3 py-2.5 font-oswald uppercase tracking-widest text-[11px] hover:bg-red-900 hover:text-white transition-all">
                      <X size={16} /> Abort Target
                    </button>
